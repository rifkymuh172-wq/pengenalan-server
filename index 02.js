...
function authMiddleware(req, res, next) {
  ...
  if (clientKey !== API_KEY) {
    return res.status(401).json({ message: "Unauthorized: API key tidak valid" });
  }
  next();
}
...
// ===== DATA SEDERHANA =====
let users = [
  { id: 1, nama: "Rifky", kelas: "D4 Elektronika" },
  { id: 2, nama: "kyy", kelas: "D4 Elektronika" },
];

// ===== Root =====
app.get("/", (req, res) => {
  res.send("Server berjalan, silakan akses endpoint API yang ada!");
});
